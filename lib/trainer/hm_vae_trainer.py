...
from lib.models.hm_vae import TwoHierSAVAEModel
...
class HMVAETrainer(nn.Module):
    def __init__(self, cfg, logger):
        super(HMVAETrainer, self).__init__()
        self.cfg = cfg
        self.logger = logger
    ...
    def forward(self, seq_rot_6d):
        ...
        return rec_seq_rot_6d, rec_seq_rot_mat, rec_seq_rot_pos, rec_seq_joint_pos, rec_seq_linear_v, loss_kl
    ...
    def rot_6d_to_rot_pos(self, seq_rot_6d):
        '''
        description: 
        param {*} seq_rot_6d: bs X T X (24*6)
        return {*}
        ...
        seq_rot_mat = hmvae_rot6d_to_rotmat(seq_rot_6d.view(bs, timesteps, n_joints, 6)) # bs X T X 24 X 3 X 3
        seq_rot_mat = seq_rot_mat.view(-1, n_joints, 3, 3) # (bs*T) X 24 X 3 X 3
        seq_rot_pos = self.fk_layer(seq_rot_mat) # (bs*T) X 24 X 3
    ...
    def visualize(self, seq_rot_6d):
        ...
        self.eval()
        with torch.no_grad():
            mean_out_rot_6d, sampled_out_rot_6d = self.model.test(seq_rot_6d) # bs X T X (24*6)
            mean_out_rot_pos = self.rot_6d_to_rot_pos(mean_out_rot_6d) # bs X T X (24*3)
            sampled_out_rot_pos = self.rot_6d_to_rot_pos(sampled_out_rot_6d) # bs X T X (24*3)
        self.train()
        return mean_out_rot_pos, sampled_out_rot_pos
    ...
    def refine_vibe(self, vibe_data_path):
        ...
        with torch.no_grad():
            ...
            for t_idx in range(0, timesteps-window_size+1, stride):
                ...
                our_rec_6d_out, _ = self.model.test(curr_encoder_input.view(1, window_size, -1)) # 1 X 64 X (24*6)
