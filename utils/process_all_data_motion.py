...
def aa2matrot(pose):
    ...
    pose_body_matrot = pose_body_matrot.view(
        batch_size, 1, -1, 3, 3)  # bs X 1 X n_joints X 3 X 3
...
def read_data(folder, sequences, dest_folder, fps=None):
    ...
    for seq_name in sequences:
        ...
        thetas, vid_names = read_single_sequence(
            seq_folder, seq_name, fk_layer, dest_folder, fps)
    ...
    for subject in tqdm(subjects, ncols=100):
        try:
            ...
            for action in actions:
                ...
                shape = np.repeat(
                    data['betas'][:10][np.newaxis], pose.shape[0], axis=0)  # N X 10
                ...
                for t_idx in range(0, timesteps, batch_size):
                    aa_data = pose[t_idx:t_idx+batch_size,
                                   :72][:, None, :]  # bs X 1 X 72
                    aa_data = aa_data.view(-1, 1, 24, 3)  # bs X 1 X 24 X 3
                    rotMatrices = aa2matrot(aa_data)  # bs X 24 X 3 X 3
                    # Convert rotation matrix to 6D representation
                    cont6DRep = torch.stack(
                        (rotMatrices[:, :, :, 0], rotMatrices[:, :, :, 1]), dim=-2)  # bs X 24 X 2 X 3
                    cont6DRep = cont6DRep.view(
                        rotMatrices.size()[0], rotMatrices.size()[1], 6)  # bs X 24 X 6
                ...
                minus_coord_data = torch.cat(
                    (coord_list[0, :, :][None, :, :], minus_coord_data), dim=0)  # T X 24 X 3
                # T X 24 X 3, the first timestep is zero, all root are zeros
                linear_v = coord_list - minus_coord_data

                # Calculate root translation
                minus_trans_data = translation[:-1, :]  # (T-1) X 3
                minus_trans_data = torch.cat(
                    (translation[0, :][None, :], minus_trans_data), dim=0)  # T X 3
                # T X 3, first timestep translation is zero
                root_v = translation - minus_trans_data
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dir', type=str, help='dataset directory',
                        default='/data/jsy/datasets/AMASS/decompress')
    ...
    parser.add_argument('--dest-folder', type=str, help='processed data directory',
                        default='/data/jsy/datasets/AMASS/amass_for_hm_vae_fps30')
