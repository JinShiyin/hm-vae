...
from lib.trainer.hm_vae_trainer import HMVAETrainer
...
if __name__ == '__main__':
    ...
    trainer = HMVAETrainer(config, logger)
    trainer.cuda()
    ...
    while True:
        ...
        for it, input_data in enumerate(train_dataset):
            ...
            if (iterations + 1) % config['visualize_iter'] == 0:
                ...
                mean_rot_pos_dir = os.path.join(image_directory, str((iterations+1)), 'mean_rot_pos')
                os.makedirs(mean_rot_pos_dir, exist_ok=True)

                sampled_rot_pos_dir = os.path.join(image_directory, str((iterations+1)), 'sampled_rot_pos')
                os.makedirs(sampled_rot_pos_dir, exist_ok=True)
                ...
                for test_it, test_input_data in enumerate(test_dataset):
                    ...
                    mean_out_rot_pos, sampled_out_rot_pos = trainer.visualize(seq_rot_6d) # bs X T X (24*3)
                    bs, timesteps, _ = seq_rot_6d.size()
                    seq_rot_pos = seq_rot_pos[0].view(1, timesteps, 24, 3)
                    mean_out_rot_pos = mean_out_rot_pos[0].view(1, timesteps, 24, 3)
                    sampled_out_rot_pos = sampled_out_rot_pos[0].view(1, timesteps, 24, 3)
                    mean_concat_seq_cmp = torch.cat([seq_rot_pos, mean_out_rot_pos], dim=0)

                    # save mean concat_seq_cmp
                    dst_mean_rot_pos_path = os.path.join(mean_rot_pos_dir, f'{test_it}.npy')
                    np.save(dst_mean_rot_pos_path, mean_concat_seq_cmp.data.cpu().numpy())
                    logger.info(f'{dst_mean_rot_pos_path} saved')

                    # save sampled seq_rot_pos
                    dst_sampled_rot_pos_path = os.path.join(sampled_rot_pos_dir, f'{test_it}.npy')
                    np.save(dst_sampled_rot_pos_path, sampled_out_rot_pos.data.cpu().numpy())
                    logger.info(f'{dst_sampled_rot_pos_path} saved')

                    torch.cuda.empty_cache()
                
                # start mean_rot_pos show3d process
                p = Process(
                    target=show3d_multi_refined_rot_pos,
                    args=(config, mean_rot_pos_dir, image_directory, iterations, 'mean_gif', logger)
                )
                p.start()
                ...
                p = Process(
                    target=show3d_multi_refined_rot_pos,
                    args=(config, sampled_rot_pos_dir, image_directory, iterations, 'sampled_gif', logger)
                )
                p.start()
