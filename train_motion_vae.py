...
output_directory = os.path.join(opts.output_path, model_name, time_stamp)
...
while True:
    ...
    for it, input_data in enumerate(train_dataset):
        loss_all, loss_kl, loss_6d_rec, loss_rot_rec, loss_pose_rec, loss_joint_pos_rec, loss_root_v_rec, loss_linear_v_rec, loss_angular_v_rec = trainer.gen_update(
            input_data, config, iterations, opts.multigpus)
        if it % 50 == 0:
            print('Training: Total loss: %.4f, KL loss: %.8f, Rec 6D loss: %.4f, Rec Rot loss: %.4f, Rec Pose loss: %.4f, Rec joint pos loss: %.4f, Rec root v loss: %.4f, Rec linear v loss: %.4f, Rec angular v loss: %.4f' % (
                loss_all, loss_kl, loss_6d_rec, loss_rot_rec, loss_pose_rec, loss_joint_pos_rec, loss_root_v_rec, loss_linear_v_rec, loss_angular_v_rec))
        ...
        if (iterations + 1) % config['validation_iter'] == 0:
            with torch.no_grad():
                for val_it, val_input_data in enumerate(val_dataset):
                    ...
                    print(
                        "*********************************************************************************************")
                    print('Val Total loss: %.4f, Val KL loss: %.8f, Val Rec 6D loss: %.4f, Val Rec Rot loss: %.4f, Val Rec Pose loss: %.4f, Val Rec joint pos loss: %.4f, Val Rec root v loss: %.4f, Val Rec linear v loss: %.4f, Val Rec angular v loss: %.4f' % (
                        val_loss_all, val_loss_kl, val_loss_6d_rec, val_loss_rot_rec, val_loss_pose_rec, val_loss_joint_pos_rec, val_loss_root_v, val_loss_linear_v, val_loss_angular_v))
